...
const INDUSTRIES = [
  { id: "ai", name: "AI・機械学習", icon: "🤖", volatility: 0.35, baseTrend: 0.02 },
  { id: "ev", name: "EV・電動モビリティ", icon: "⚡", volatility: 0.3, baseTrend: 0.015 },
  { id: "space", name: "宇宙開発", icon: "🚀", volatility: 0.4, baseTrend: 0.01 },
  { id: "biotech", name: "バイオテクノロジー", icon: "🧬", volatility: 0.35, baseTrend: 0.01 },
  { id: "metaverse", name: "メタバース・XR", icon: "🥽", volatility: 0.4, baseTrend: -0.005 },
  { id: "fintech", name: "フィンテック", icon: "💳", volatility: 0.25, baseTrend: 0.01 },
  { id: "greentech", name: "環境・クリーンエネルギー", icon: "🌱", volatility: 0.3, baseTrend: 0.02 },
  { id: "food", name: "フードテック", icon: "🍔", volatility: 0.2, baseTrend: 0.005 },
  { id: "security", name: "サイバーセキュリティ", icon: "🛡️", volatility: 0.25, baseTrend: 0.015 },
  { id: "drone", name: "ドローン・ロボティクス", icon: "🦾", volatility: 0.3, baseTrend: 0.01 },
  { id: "medical", name: "デジタル医療", icon: "🏥", volatility: 0.3, baseTrend: 0.015 },
  { id: "realestate", name: "不動産テック", icon: "🏢", volatility: 0.2, baseTrend: 0.005 },
  { id: "gaming", name: "ゲーム・eスポーツ", icon: "🎮", volatility: 0.3, baseTrend: 0.01 },
  { id: "logistics", name: "物流DX", icon: "📦", volatility: 0.2, baseTrend: 0.01 },
  { id: "edu", name: "EdTech", icon: "📚", volatility: 0.2, baseTrend: 0.005 },
];
...
const BETRAYAL_MSGS = [
  ...
  "「CEOには悪いけど、俺のほうが上手くやれる」と言い残した。目は笑っていなかった",
...
function generateBreakroomChats(subs, companies) {
  ...
  const OFFICE_GOSSIP = [
    ...
    (a, b) => [
      ...
      { name: b.name, text: "いや俺ね、社食のカレーにはこだわりがあって" },
    ...
    (a, b) => [
      ...
      { name: b.name, text: "俺この前7階で沈黙に耐えきれなくて「あ、ここで降ります」って言っちゃった" },
  ...
  const TRAIT_CHAT = [
    ...
    (a, b) => b.trait.name === "カリスマ" ? [
      ...
      { name: b.name, text: "でもさ、俺だって家帰ったら何もできないよ。洗い物とか溜まってるし" },
  ...
  if (Math.random() < 0.6) {
    ...
    const c = others.length > 0 ? pick(others) : b;
...
const STAT_LABELS = {
  leadership: { name: "統率力", icon: "👑" },
  execution: { name: "実行力", icon: "⚙️" },
  creativity: { name: "創造力", icon: "💡" },
  negotiation: { name: "交渉力", icon: "🤝" },
  stamina: { name: "体力", icon: "💪" },
  luck: { name: "運", icon: "🎲" },
  loyalty: { name: "忠誠心", icon: "🫡" },
  ambition: { name: "野心", icon: "🔥" },
};
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                    {chat.msgs.map((msg, mi) => (
                      <div key={mi} style={{ display: "flex", gap: 8, alignItems: "start" }}>
                        <span style={{
                          fontSize: 11, color: "#8a7a6a", fontWeight: 700,
